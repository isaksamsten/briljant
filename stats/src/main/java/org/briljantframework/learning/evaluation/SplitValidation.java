...
/**
 * Perform a hold-out cross-validation using a percentage split. The default split is 33 percent for
 * testing and 67% for training.
 * <p>
...
public class SplitValidation extends AbstractEvaluator {
  ...
  public SplitValidation(double testRatio) {
    ...
    Preconditions.checkArgument(testRatio > 0 && testRatio < 1,
        "Test ratio must be in range )0, 1(.");
  ...
  public Result evaluate(Classifier classifier, DataFrame x, Vector y) {
    ...
    // SupervisedDatasetSplit<D, T> partitions =
    // SupervisedDatasetSplit.withFraction(supervisedDataset, testRatio);
    ...
    //
    // ConfusionMatrix confusionMatrix = ConfusionMatrix.create(outSamplePredictions,
    // test.getTarget());
    // List<Metric> metrics =
