...
public class Result implements Chartable {
    ...
    public static Result create(List<Metric> metrics, List<ConfusionMatrix> confusionMatrices) {
        ...
        if (metrics.size() > 0) {
            ...
            if (confusionMatrices.size() != length) {
                throw new IllegalArgumentException("ConfusionMatrix don't have the same number of values as the " +
                        "metrics");
            }
        }
        ...
        for (Metric metric : metrics) {
            if (metric.size() != length) {
                throw new IllegalArgumentException(String.format("Invalid number of metrics for %s", metric.getName()));
            }
    ...
    public String toString() {
        ...
        builder.append("Average Confusion Matrix\n")
                .append(getAverageConfusionMatrix())
                .append("\n\n")
                .append("Metrics\n");
        ...
        for (Metric metric : getMetrics()) {
            ...
            table.put("Standard Deviation", metric.getName(), String.format("%.4f", metric.getStandardDeviation()));
        }
