 ...
 * <p>
 * TODO(isak): Provide parameter for calculating in-sample error rates. Might be quite costly.
 *
 * @param <T> the type parameter
 */
public class CrossValidation<D extends DataFrame<?>, T extends Column> extends AbstractEvaluator<D, T> {
     ...
     */
    public static <D extends DataFrame<?>, T extends Column> CrossValidation<D, T> create() {
     ...
     * @return the cross validation evaluator
     */
    public static <D extends DataFrame<?>, T extends Column> CrossValidation<D, T> withFolds(int folds) {
    ...
    public Result evaluate(Classifier<?, ? super D, ? super T> classifier, SupervisedDataset<? extends D, ? extends T> supervisedDataset) {
        SupervisedDatasetPartitions<D, T> partitions = SupervisedDatasetPartitions.create(supervisedDataset, folds);
        ...
        for (int i = 0; i < folds; i++) {
            ...
            Model<?, ? super D> model = classifier.fit(trainingSet.getDataFrame(), trainingSet.getTarget());
            ...
            confusionMatrices.add(ConfusionMatrix.create(outSamplePredictions, validationSet.getTarget()));
