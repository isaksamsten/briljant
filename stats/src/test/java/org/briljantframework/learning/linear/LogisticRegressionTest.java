...
//        Transformer<Frame, DefaultTarget> pipeline = PipelineTransformer.of(new RemoveIncompleteCases(),
//                new ZNormalizer());
...
//
//        System.out.println(result.getMetrics().stream().map(x -> x.getAverage(Metric.Sample.IN)).collect(Collectors
//                .toList()));
//
//
...
//        Pipeline p = Pipeline.with(InputDataset.withFile("erlang/test.txt").unboxer(new RuleDiscoveryUnboxer<>
// (Matrix.factory())))
