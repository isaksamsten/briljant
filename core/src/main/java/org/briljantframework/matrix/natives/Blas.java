...
public class Blas {
  ...
  /*
   * This is a rather ad-hoc search for the correct library to load. This must be tested on a wide
   ...
   * <p>
   * On return, the contents of vector Y are replaced with the result. The value computed is (alpha
   * * X[i]) + Y[i].
   *
   ...
   * @param incY Stride within Y. For example, if incY is 7, every 7th element is used.
   */
  public static native void cblas_daxpy(int N, double alpha, double[] X, int incX, double[] Y,
      int incY);
   ...
   * @param incY the inc y
   */
  public static native void cblas_dgemv(int Order, int TransA, int M, int N, double alpha,
   ...
   * @param ldc the ldc
   */
  public static native void cblas_dgemm(int Order, int TransA, int TransB, int M, int N, int K,
