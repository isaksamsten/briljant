...
public class LinearAlgebra {
    ...
    /**
     * In statistics and mathematics, linear least squares is an approach fitting a mathematical or statistical model to
     * data in cases where the idealized value provided by the model for any data point is expressed linearly in terms
     * of the unknown parameters of the model. The resulting fitted model can be used to summarize the data, to predict
    ...
    /**
     * A computationally simple and accurate way to compute the pseudo inverse is by using the singular value
     * decomposition. If A = U\Sigma V^* is the singular value decomposition of A, then A^+ = V\Sigma^+ U^*. For a
     * diagonal matrix such as \Sigma, we get the pseudo inverse by taking the reciprocal of each non-zero element on
     * the diagonal, leaving the zeros in place. In numerical computation, only elements larger than some small
     * tolerance are taken to be nonzero, and the others are replaced by zeros.
     *
     ...
     * \mathbf{M} = \mathbf{U} \boldsymbol{\Sigma} \mathbf{V}^* where U is a m×m real or complex unitary matrix, \Sigma
     * is an m×n rectangular diagonal matrix with nonnegative real numbers on the diagonal, and V* (the conjugate
     * transpose of V, or simply the transpose of V if V is real) is an n×n real or complex unitary matrix. The diagonal
     * entries \Sigma_{i,i} of \Sigma are known as the singular values of M. The m headers of U and the n headers of V
     * are called the left-singular vectors and right-singular vectors of M, respectively.
     *
     * @param matrix the matrix
     ...
     * called principal components. The number of principal components is less than or equal to the number of original
     * variables. This transformation is defined in such a way that the first principal component has the largest
     * possible variance (that is, accounts for as much of the variability in the data as possible), and each succeeding
     * component in turn has the highest variance possible under the constraint that it is orthogonal to (i.e.,
     * uncorrelated with) the preceding components. Principal components are guaranteed to be independent if the data
    ...
    /**
     * In linear algebra, the determinant is a value associated with a square matrix. It can be computed from the
     * entries of the matrix by a specific arithmetic expression, while other ways to determine its value exist as well.
     * The determinant provides important information about a matrix of coefficients of a system of linear equations, or
     * about a matrix that corresponds to a linear transformation of a vector space.
     *
    ...
    /**
     * In linear algebra, the rank of a matrix A is the size of the largest collection of linearly independent columns
     * of A (the column rank) or the size of the largest collection of linearly independent rows of A (the row rank).
     * For every matrix, the column rank is equal to the row rank.[1] It is a measure of the "nondegenerateness" of the
     * system of linear equations and linear transformation encoded by A. There are multiple definitions of rank. The
     * rank is one of the fundamental pieces of data associated with a matrix.
     *
     * @param x a matrix
     * @return the rank
     */
