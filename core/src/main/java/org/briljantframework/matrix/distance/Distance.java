...
/**
 * In mathematics, a metric or distance function is a function that defines a distance between elements of a set. A set
 * with a metric is called a metric space. A metric induces a topology on a set but not all topologies can be generated
 * by a metric. A topological space whose topology can be described by a metric is called metrizable. A metric on a set
 * X is a function (called the distance function or simply distance)
 * <p>
 * d : X × X → R
 * <p>
 * (where R is the set of real numbers). For all x, y, z in X, this function is required to satisfy the following
 * conditions:
 * <p>
 * <pre>
 ...
 * </pre>
 * <p>
 * Users of this class should not assume that all conditions are fulfilled. In some cases only the relative order is
 * important.
 * <p>
