 ...
 * (that is, accounts for as much of the variability in the data as possible), and each succeeding component in turn has
 * the highest variance possible under the constraint that it is orthogonal to (i.e., uncorrelated with) the preceding
 * components. Principal components are guaranteed to be independent if the data set is jointly normally distributed.
...
public class PrincipalComponentAnalyzer
    ...
    private SingularValueDecomposition getSingularValueDecomposition(Matrix array) {
        Matrix sigma = Matrices.mmul(DenseMatrix::new, array, Transpose.YES, 1.0, array, Transpose.NO,
                1.0 / array.rows());
