 ...
 * \mathbf{M} = \mathbf{U} \boldsymbol{\Sigma} \mathbf{V}^* where U is a m×m real or complex unitary matrix, \Sigma is
 * an m×n rectangular diagonal matrix with nonnegative real numbers on the diagonal, and V* (the conjugate transpose of
 * V, or simply the transpose of V if V is real) is an n×n real or complex unitary matrix. The diagonal entries
 * \Sigma_{i,i} of \Sigma are known as the singular values of M. The m headers of U and the n headers of V are called
