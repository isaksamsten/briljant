...
/**
 * In time series analysis, dynamic time warping (DTW) is an algorithm for measuring similarity between two temporal
 ...
 * certain restrictions. The sequences are "warped" non-linearly in the time dimension to determine a measure of their
 * similarity independent of certain non-linear variations in the time dimension. This sequence alignment method is
 * often used in time series classification. Although DTW measures a distance-like quantity between two given sequences,
 * it doesn't guarantee the triangle inequality to hold.
 * <p>
 * Specifically note that this implementation is NOT - I repeat - NOT, thread safe. Create one {@link
 * DynamicTimeWarping}** object for every thread! This way, the number of created matrices equals the number of threads,
 * not the number of computations of {@link #distance(double, double)}.
 * <p>
    ...
    public double distance(MatrixLike a, MatrixLike b) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            for (int j = start; j < end; j++) {
                ...
                dwt.put(i, j, cost + Scalars.min(dwt.get(i - 1, j), dwt.get(i, j - 1), dwt.get(i - 1, j - 1)));
            }
        }
